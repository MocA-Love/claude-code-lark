...
Larkは接続中のクライアントの1つにだけランダムにメッセージを送信する。同じLarkアプリで複数のClaude Codeセッションを起動すると、メッセージが意図したセッションに届かない場合がある。並列セッションには別々のLarkアプリを使用すること。

### ゾンビ接続

`kill -9`（SIGKILL）でセッションが終了した場合、WebSocket接続を正常に閉じられない。Larkのpingタイムアウト（推定4〜6分）が経過するまで、ゾンビ接続にメッセージが送られ続ける。通常の終了（Ctrl+C、`kill`）では接続は正しく閉じられる。
